...
@Composable
fun HomeBody(
    source: RequestState<Currency>, target: RequestState<Currency>, amount: Double
) {
    var exchangeAmount by rememberSaveable { mutableStateOf(0.0) }
    val animatedExchangeAmount by animateValueAsState(
        targetValue = exchangeAmount, animationSpec = tween(300), typeConverter = DoubleConverter()
    )
